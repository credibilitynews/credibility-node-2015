...
class Typeahead extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        if(!this.state.searching && nextProps.options && nextProps.options.length > 0){
    ...
    renderOption(item, isHighlighted) {
        return (
            <div className={isHighlighted ? 'typeahead-option highlighted' : 'typeahead-option'}
                key={item.id}
                id={item.id}>
                {item.label}
            </div>);
    ...
    _handleInputChange(e, input) {
        if(this.searchRequest
            && this.searchRequest.abort) this.searchRequest.abort();

        if(input != null && input.length > 2){
            this.searchRequest = this.props.fetchOptionsAction(input, (err, result) =>{
