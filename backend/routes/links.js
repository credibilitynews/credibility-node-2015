...
module.exports = [
  {
    route:
      "linksById[{integers:linkIds}]['id', 'title', 'url', 'created_at', 'updated_at', 'views', 'user_id', 'topic_id', 'bias', 'author_id', 'news_agency_id', 'content_type', 'topic_title', 'user_name']",
  ...
  {
    route:
      "latestLinks[{integers:n}]['id', 'title', 'url', 'created_at', 'updated_at', 'views', 'user_id', 'user_name', 'topic_id', 'topic_title', 'bias', 'author_id', 'news_agency_id', 'content_type']",
    get: function (pathSet) {
        ...
        .then(function (links) {
          ...
          pathSet.n.forEach(function (n, index) {
            ...
            fields.forEach(function (field) {
              var value =
                linkRecord && linkRecord[field]
                  ? linkRecord[field].toString()
                  : undefined;
  ...
  {
    route:
      "taggedLinks[{integers:n}]['id', 'title', 'url', 'created_at', 'updated_at', 'views', 'user_id', 'user_name', 'topic_id', 'topic_title', 'bias', 'author_id', 'news_agency_id', 'content_type', 'tag_name']",
  ...
  {
    route:
      "linksByTopicId[{integers:n}]['id', 'title', 'url', 'created_at', 'views', 'user_id', 'user_name', 'topic_id', 'topic_title', 'bias', 'author_id', 'news_agency_id', 'content_type', 'tag_name']",
    call: function (pathSet, args) {
      var limit = pathSet.n.slice(-1)[0] - pathSet.n[0] + 1;
      var offset = pathSet.n[0];
  ...
  {
    route:
      "fetchUrlMeta['title', 'author', 'image', 'article', 'hashtags', 'polarity', 'subjectivity']",
    call: function (pathSet, args) {
      return linkService
        .fetchUrlMeta(args[0])
        .then((meta) => {
          ...
          results.push({
            ...
            value:
              meta.sentiment.polarity +
              " (" +
              meta.sentiment.polarity_confidence +
              ")",
          });
          results.push({
            path: ["fetchUrlMeta", "subjectivity"],
            value:
              meta.sentiment.subjectivity +
              " (" +
              meta.sentiment.subjectivity_confidence +
              ")",
          });
